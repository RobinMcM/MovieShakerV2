...
function ArtifactThumbnail({ url, seekTo, isImage }: { url?: string; seekTo?: number; isImage?: boolean }) {
  if (!url) {
    return (
  ...
  return (
    <video
      ...
      onLoadedMetadata={(e) => { e.currentTarget.currentTime = seekTo ?? 0; }}
    />
  );
}

// ---------------------------------------------------------------------------
// ArtifactsPanel — persistent right sidebar with visual thumbnail cards
// ---------------------------------------------------------------------------
...
function EditorView({ projectId }: { projectId: string }) {
  ...
  function playPointToPixel(clipIdx: number, timeOffset: number): number {
    const GAP = 12;
    let px = GAP;
    for (let i = 0; i < clipIdx && i < timeline.video_track.length; i++) {
      px += blockWidth(timeline.video_track[i].duration) + GAP;
    }
    if (clipIdx < timeline.video_track.length) {
      const item = timeline.video_track[clipIdx];
      px += (timeOffset / item.duration) * blockWidth(item.duration);
    }
    return px;
  }
                ...
                {!previewItem ? (
                  ...
                  <video
                    ...
                    onTimeUpdate={() => {
                      ...
                      if (!vid) return;
                      const baseTime = previewItem.in_time ?? 0;
                      const timeOffset = Math.max(0, vid.currentTime - baseTime);
                      setPlayStartX(playPointToPixel(playingVideoIdx, timeOffset));
                      if (previewItem.out_time != null && vid.currentTime >= previewItem.out_time && !vid.paused) {
                        vid.pause();
                        handleVideoEnded();
                      }
                    }}
